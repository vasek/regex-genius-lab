...
import { Agent, TestCases, GenerationState, Iteration } from '../types';
...
const simulateGenerationProcess = async (): Promise<void> => {
  try {
    for (let iterationNumber = 1; iterationNumber <= generationState.maxIterations; iterationNumber++) {
      ...
      await delay(1200);
      updateAgentState(iterationNumber, 3, 'completed', generateMockRegex(), Math.floor(70 + Math.random() * 30));
      
      // Step 3: Agent 1 evaluates regexes
      await delay(1500);
      updateAgentState(iterationNumber, 2, 'completed', undefined, Math.floor(70 + Math.random() * 30));
      updateAgentState(iterationNumber, 1, 'completed', generateMockRegex(), Math.floor(70 + Math.random() * 30));
...
const updateAgentState = (
  ...
  const iterationObj = generationState.iterations.find(it => it.number === iteration);
